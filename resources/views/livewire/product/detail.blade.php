<div class="bg-white">
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                <h3 class="text-lg font-medium text-gray-900">
                                    <a href="{{ route('profile.show', $product->seller->id) }}" class="hover:underline hover:text-primary-600">{{ $product->seller->name }}</a>
                                </h3>
        ...
        <section aria-labelledby="reviews-heading" class="mt-16 sm:mt-24">
            <h2 id="reviews-heading" class="text-lg font-medium text-gray-900">Customer Reviews</h2>
            <div class="mt-6 border-t border-gray-200 pt-10 pb-10">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    @foreach($product->reviews as $review)
                        <div class="bg-gray-50 p-6 rounded-lg">
                            <div class="flex items-center mb-4">
                                @if($review->reviewer->avatar)
                                    <img src="{{ $review->reviewer->avatar }}" alt="{{ $review->reviewer->name }}" class="w-10 h-10 rounded-full mr-4">
                                @else
                                    <div class="w-10 h-10 rounded-full bg-primary-100 flex items-center justify-center text-primary-700 font-bold mr-4">
                                        {{ substr($review->reviewer->name, 0, 1) }}
                                    </div>
                                @endif
                                <div>
                                    <h4 class="text-sm font-bold text-gray-900">
                                        <a href="{{ route('profile.show', $review->reviewer->id) }}" class="hover:underline hover:text-primary-600">{{ $review->reviewer->name }}</a>
                                    </h4>
                                    <div class="flex items-center mt-1">
                                        ...
                                        <p class="ml-2 text-xs text-gray-500">{{ $review->created_at->format('M d, Y') }}</p>
                                    </div>
                                </div>
                            </div>
