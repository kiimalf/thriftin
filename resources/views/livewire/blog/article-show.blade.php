<div>
    ...
    <div class="bg-white pt-16 pb-8 sm:pt-24 sm:pb-12">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <span class="inline-flex items-center px-3 py-1 rounded-lg text-sm font-medium bg-primary-50 text-primary-600 mb-6">
                Blog
            </span>
            <h1 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl mb-6">{{ $article->title }}</h1>
            
            @if($article->excerpt)
            <p class="text-lg text-gray-500 mb-8">{{ $article->excerpt }}</p>
            @endif
            ...
            <div class="flex items-center justify-center">
                ...
                @else
                    <div class="h-12 w-12 rounded-full bg-primary-50 text-primary-600 flex items-center justify-center font-bold text-lg mr-4">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
        <div class="rounded-2xl overflow-hidden bg-gray-50" style="aspect-ratio: 16/9; max-height: 480px;">
    ...
    <div class="bg-white pb-16 sm:pb-24">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-16 pt-8 border-t border-gray-100">
                <a href="{{ route('blog.index') }}" class="inline-flex items-center font-medium text-primary-600 hover:text-primary-700 transition-colors">
                    <svg class="mr-2 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 16l-4-4m0 0l4-4m-4 4h18"></path></svg>
    ...
    <div class="bg-gray-50/50 py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 class="text-2xl font-bold text-gray-900 mb-8">Read next</h2>
                ...
                <article class="flex flex-col bg-white rounded-2xl border border-gray-100 overflow-hidden card-hover">
                    @if($related->image)
                        <div class="flex-shrink-0 overflow-hidden">
                            <img class="h-48 w-full object-cover img-zoom" src="{{ $related->image_url }}" alt="{{ $related->title }}">
                        </div>
                    @else
                    ...
                    <div class="flex-1 p-6 flex flex-col justify-between">
                        <div class="flex-1">
                            <a href="{{ route('blog.show', $related->slug) }}" class="block mt-2">
                                <p class="text-lg font-bold text-gray-900 line-clamp-2 hover:text-primary-600 transition-colors">{{ $related->title }}</p>
                                <p class="mt-3 text-sm text-gray-500 line-clamp-3">{{ $related->excerpt ?? Str::limit(strip_tags($related->content), 120) }}</p>
                            </a>
                        </div>
